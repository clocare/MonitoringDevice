...
void GPIO_voidSetPortPinValue(t_PORT cpy_port , t_PIN cpy_pin , uint8 cpy_value){
    switch (cpy_port)
    {
    case PORTA:
        if(cpy_value == 0){
...
void GPIO_voidSetPortValue(t_PORT cpy_port , uint16 cpy_value){
...
uint8 GPIO_u8GetPortPinValue(t_PORT cpy_port , t_PIN cpy_pin){
...
uint16 GPIO_u16GetPortValue(t_PORT cpy_port){
...
void GPIO_voidSetPullType (t_PORT cpy_port , t_PIN cpy_pin , uint8 pull_type){
    if (pull_type == 1){
        switch (cpy_port)
        {
        case PORTA:
...
{
    uint8 local_reg_idx  = cpy_pin / 4;            // Select register in the array
    uint8 local_line_bit = (cpy_pin % 4) * 4;      // Select bits numbers
    uint8 local_port_map = PORTA_MAP;

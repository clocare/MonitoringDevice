 ...
 */
uint8 MIFARE_TwoStepHelper(	uint8 command,	///< The command to use
									uint8 blockAddr,	///< The block (0-0xff) number.
									long data		///< The data to transfer in step 2
									) {
	uint8 result;
	uint8 cmdBuffer[2]; // We only need room for 2 bytes.
	...
	result = PCD_MIFARE_Transceive(	(uint8 *)&data, 4, TRUE); // Adds CRC_A and accept timeout as success.
	if (result != mfrc522_STATUS_OK) {
		return result;
	}
...
/* Public functions ----------------------------------------------------------*/
void PCD_WriteRegister(	uint8 reg,		///< The register to write to. One of the PCD_Register enums.
									uint8 value		///< The value to write.
								) {
		// Select slave
 ...
 */
void PCD_WriteRegisterMulti(	uint8 reg,		///< The register to write to. One of the PCD_Register enums.
									uint8 count,		///< The number of bytes to write to the register
									uint8 *values	///< The values to write. Byte array.
								) {
	// Select slave
	GPIO_voidSetPortPinValue(mfrc522_CS_PORT , mfrc522_CS_PIN , STD_LOW);	
	SPI2_u8SendRecieveSync(reg & 0x7E);				// MSB == 0 is for writing. LSB is not used in address. Datasheet section 8.1.2.3.
...
/**
 * Reads a uint8 from the specified register in the MFRC522 chip.
 * The interface is described in the datasheet section 8.1.2.
 */
uint8 PCD_ReadRegister(	uint8 reg	///< The register to read from. One of the PCD_Register enums.
								) {
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void PCD_ReadRegisterMulti(	uint8 reg,		///< The register to read from. One of the PCD_Register enums.
								uint8 count,		///< The number of bytes to read
								uint8 *values,	///< Byte array to store the values in.
								uint8 rxAlign	///< Only bit positions rxAlign..7 in values[0] are updated.
								) {
	if (count == 0) {
		return;
	}
	//printf("Reading %d bytes from register.", count);
	uint8 address = 0x80 | (reg & 0x7E);		// MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
 ...
 */
void PCD_SetRegisterBitMask(	uint8 reg,	///< The register to update. One of the PCD_Register enums.
 ...
 */
void PCD_ClearRegisterBitMask(	uint8 reg,	///< The register to update. One of the PCD_Register enums.
 ...
 * @return mfrc522_STATUS_OK on success, mfrc522_STATUS_??? otherwise.
 */
uint8 PCD_CalculateCRC(	uint8 *data,		///< In: Pointer to the data to transfer to the FIFO for CRC calculation.
								uint8 length,	///< In: The number of bytes to transfer.
								uint8 *result	///< Out: Pointer to result buffer. Result is written to result[0..1], low uint8 first.
					 ) {
...
void PCD_AntennaOn(void) {
	uint8 value = PCD_ReadRegister(TxControlReg);
	if ((value & 0x03) != 0x03) {
...
uint8 PCD_TransceiveData(	uint8 *sendData,		///< Pointer to the data to transfer to the FIFO.
									...
									uint8 *backData,		///< NULL_PTR or pointer to buffer if data should be read back after executing the command.
									uint8 *backLen,		///< In: Max number of bytes to write to *backData. Out: The number of bytes returned.
									uint8 *validBits,	///< In/Out: The number of valid bits in the last uint8. 0 for 8 valid bits. Default NULL_PTR.
...
uint8 PCD_CommunicateWithPICC(	uint8 command,		///< The command to execute. One of the PCD_Command enums.
	...
	uint8 bitFraming	= (uint8)(rxAlign << 4) + txLastBits;		// RxAlign = BitFramingReg[6..4]. TxLastBits = BitFramingReg[2..0]
 ...
 * @return mfrc522_STATUS_OK on success, mfrc522_STATUS_??? otherwise.
 */
uint8 PICC_RequestA(uint8 *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
							uint8 *bufferSize	///< Buffer size, at least two bytes. Also number of bytes returned if mfrc522_STATUS_OK.
							) {
 ...
 * @return mfrc522_STATUS_OK on success, mfrc522_STATUS_??? otherwise.
 */
uint8 PICC_WakeupA(	uint8 *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
							uint8 *bufferSize	///< Buffer size, at least two bytes. Also number of bytes returned if mfrc522_STATUS_OK.
							) {
 ...
 * @return mfrc522_STATUS_OK on success, mfrc522_STATUS_??? otherwise.
 */
uint8 PICC_REQA_or_WUPA(	uint8 command, 		///< The command to send - PICC_CMD_REQA or PICC_CMD_WUPA
	...
	validBits = 7;										// For REQA and WUPA we need the short frame format - transmit only 7 bits of the last (and only) uint8. TxLastBits = BitFramingReg[2..0]
 ...
 * @return mfrc522_STATUS_OK on success, mfrc522_STATUS_??? otherwise.
 */
uint8 PICC_Select(	Uid *uid,			///< Pointer to Uid struct. Normally output, but can also be used to supply a known UID.
	...
	uint8 buffer[9];					// The SELECT/ANTICOLLISION commands uses a 7 uint8 standard frame + 2 bytes CRC_A
	...
	uidComplete = FALSE;
	while ( ! uidComplete) {
		// Set the Cascade Level in the SEL uint8, find out if we need to use the Cascade Tag in uint8 2.
		switch (cascadeLevel) {
			case 1:
		...
		uint8 bytesToCopy = currentLevelKnownBits / 8 + (currentLevelKnownBits % 8 ? 1 : 0); // The number of bytes needed to represent the known bits for this level.
		if (bytesToCopy) {
			uint8 maxBytes = useCascadeTag ? 3 : 4; // Max 4 bytes in each Cascade Level. Only 3 left if we use the Cascade Tag
 ...
 */
uint8 PCD_Authenticate(uint8 command,		///< PICC_CMD_MF_AUTH_KEY_A or PICC_CMD_MF_AUTH_KEY_B
	...
	for (uint8 i = 0; i < MF_KEY_SIZE; i++) {	// 6 key bytes
		sendData[2+i] = key->keyByte[i];
	}
	for (uint8 i = 0; i < 4; i++) {				// The first 4 bytes of the UID
 ...
 * @return mfrc522_STATUS_OK on success, mfrc522_STATUS_??? otherwise.
 */
uint8 MIFARE_Read(	uint8 blockAddr, 	///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The first page to return data from.
							uint8 *buffer,		///< The buffer to store the data in
							uint8 bufferSize	///< Buffer size, at least 18 bytes. Also number of bytes returned if mfrc522_STATUS_OK.
						) {
	uint8 result;
 ...
 * @return mfrc522_STATUS_OK on success, mfrc522_STATUS_??? otherwise.
 */
uint8 MIFARE_Write(	uint8 blockAddr, ///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The page (2-15) to write to.
...
/**
 * Writes a 4 uint8 page to the active MIFARE Ultralight PICC.
 *
 * @return mfrc522_STATUS_OK on success, mfrc522_STATUS_??? otherwise.
 */
uint8 MIFARE_Ultralight_Write(	uint8 page, 		///< The page (2-15) to write to.
										uint8 *buffer,	///< The 4 bytes to write to the PICC
										uint8 bufferSize	///< Buffer size, must be at least 4 bytes. Exactly 4 bytes are written.
									) {
	uint8 result;
 ...
 * @return mfrc522_STATUS_OK on success, mfrc522_STATUS_??? otherwise.
 */
uint8 MIFARE_Decrement(	uint8 blockAddr, ///< The block (0-0xff) number.
 ...
 * @return mfrc522_STATUS_OK on success, mfrc522_STATUS_??? otherwise.
 */
uint8 MIFARE_Increment(	uint8 blockAddr, ///< The block (0-0xff) number.
 ...
 * @return mfrc522_STATUS_OK on success, mfrc522_STATUS_??? otherwise.
 */
uint8 MIFARE_Restore(	uint8 blockAddr ///< The block (0-0xff) number.
							) {
 ...
 * @return mfrc522_STATUS_OK on success, mfrc522_STATUS_??? otherwise.
 */
uint8 MIFARE_Transfer(	uint8 blockAddr ///< The block (0-0xff) number.
							) {
	uint8 result;
	uint8 cmdBuffer[2]; // We only need room for 2 bytes.
 ...
 * @return mfrc522_STATUS_OK on success, mfrc522_STATUS_??? otherwise.
 */
uint8 PCD_MIFARE_Transceive(	uint8 *sendData,		///< Pointer to the data to transfer to the FIFO. Do NOT include the CRC_A.
 ...
 *
 */
const char *GetStatusCodeName(uint8 code	///< One of the StatusCode enums.
										) {
	switch (code) {
 ...
 * @return PICC_Type
 */
uint8 PICC_GetType(uint8 sak		///< The SAK uint8 returned from PICC_Select().
							) {
...
uint8 PCD_DumbVersion(void)
{
	uint8 v = PCD_ReadRegister(VersionReg);
	return v; 
}
/**
 ...
 *
 */
const char *PICC_GetTypeName(uint8 piccType	///< One of the PICC_Type enums.
										) {
	switch (piccType) {
...
void PICC_DumpToSerial(Uid *uid	///< Pointer to Uid struct returned from a successful PICC_Select().
	...
	// PICC type
	uint8 piccType = PICC_GetType(uid->sak);
...
void PICC_DumpMifareClassicToSerial(	Uid *uid,		///< Pointer to Uid struct returned from a successful PICC_Select().
												uint8 piccType,	///< One of the PICC_Type enums.
												MIFARE_Key *key	///< Key A used for all sectors.
											) {
	uint8 no_of_sectors = 0;
	switch (piccType) {
		case PICC_TYPE_MIFARE_MINI:
...
void PICC_DumpMifareClassicSectorToSerial(Uid *uid,			///< Pointer to Uid struct returned from a successful PICC_Select().
													...
													uint8 sector			///< The sector to dump, 0..39.
													) {
	uint8 status;
	uint8 firstBlock;		// Address of lowest address to dump actually last block dumped)
	uint8 no_of_blocks;		// Number of blocks in sector
	...
	uint8 c1, c2, c3;		// Nibbles
	uint8 c1_, c2_, c3_;		// Inverted nibbles
	boolean invertedError;		// True if one of the inverted nibbles did not match
	uint8 g[4];				// Access bits for each of the four groups.
	uint8 group;				// 0-3 - active group for access bits
...
void PICC_DumpMifareUltralightToSerial() {
	...
	for (uint8 page = 0; page < 16; page +=4) { // Read returns data for 4 pages at a time.
		// Read pages
		byteCount = sizeof(buffer);
 ...
 */
void MIFARE_SetAccessBits(	uint8 *accessBitBuffer,	///< Pointer to uint8 6, 7 and 8 in the sector trailer. Bytes [0..2] will be set.
									uint8 g0,				///< Access bits [C1 C2 C3] for block 0 (for sectors 0-31) or blocks 0-4 (for sectors 32-39)
									uint8 g1,				///< Access bits C1 C2 C3] for block 1 (for sectors 0-31) or blocks 5-9 (for sectors 32-39)
									uint8 g2,				///< Access bits C1 C2 C3] for block 2 (for sectors 0-31) or blocks 10-14 (for sectors 32-39)
									uint8 g3					///< Access bits C1 C2 C3] for the sector trailer, block 3 (for sectors 0-31) or block 15 (for sectors 32-39)
								) {
	uint8 c1 = (uint8)((g3 & 4) << 1) | (uint8)((g2 & 4) << 0) | (uint8)((g1 & 4) >> 1) | (uint8)((g0 & 4) >> 2);
	uint8 c2 = (uint8)((g3 & 2) << 2) | (uint8)((g2 & 2) << 1) | (uint8)((g1 & 2) << 0) | (uint8)((g0 & 2) >> 1);
	uint8 c3 = (uint8)((g3 & 1) << 3) | (uint8)((g2 & 1) << 2) | (uint8)((g1 & 1) << 1) | (uint8)((g0 & 1) << 0);

	accessBitBuffer[0] = (uint8)((~c2 & 0xF) << 4 )| (~c1 & 0xF);
	accessBitBuffer[1] = (uint8)(c1 << 4) | (~c3 & 0xF);
	accessBitBuffer[2] = (uint8)(c3 << 4) | c2;
...
boolean PICC_IsNewCardPresent() 
{
	...
	uint8 result = PICC_RequestA(bufferATQA, &bufferSize);
...
int PICC_ReadBlock(int blockNumber, uint8 arrayAddress[], MIFARE_Key *key , Uid * uid)
{
 ...
 //        printf("PCD_Authenticate() failed (read): %s\r\n", GetStatusCodeName(status));
  ...
  uint8 buffersize = 18;//we need to define a variable with the read buffer size, since the MIFARE_Read method below needs a pointer to the variable that contains the size...
